...
Outro importante conectivo lógico é a quantificação existencial. Para dizer que há algum x do tipo t tal que alguma propriedade p é verdadeira para x, escrevemos (Sigma t (x => p x)). A anotação de tipo: t pode ser omitida se o Kind for capaz de inferir do contexto qual o tipo de x deve ser.
...
Para provar uma afirmação da forma ($ x p), devemos mostrar que p é verdadeira para alguma escolha específica de valor para x, conhecido como testemunho do existencial. Isso é feito em duas etapas: Primeiro, explicitamente informamos ao Kind qual testemunho x temos em mente escrevendo-o no primeiro parâmetro $. Então, provamos que p é verdadeira depois que todas as ocorrências de x são substituídas por t.
...
Por outro lado, se temos uma hipótese existencial (x ** p) no contexto, podemos fazer um pattern match nela para obter um testemunho x e uma hipótese afirmando que p é verdadeira para x.
